...
```text
concept-board skill로 현재 repo의 Git local/remote 흐름을 시각화해주세요.
concept-board skill로 이번 작업의 decision map을 만들어주세요.
concept-board skill로 work-map.md와 mission-backlog.md를 기반으로 자동화 후보 보드를 만들어주세요.
concept-board skill로 이 repo의 architecture/data flow를 HTML 보드로 보여주세요.
```
